<script lang="ts">
    ...
    let selectedChapterInformation = $derived(chapters.find(chapter => chapter.number === selectedChapterNumber));
	...
	<div class="chapter-container">
		<ul>
			{#each chapters as chapter, index}
				<li>
					<button class="chapter-title" class:chapter-title-selected = {selectedChapterNumber === chapter.number} onclick={() => handleSelectChapter(chapter)} aria-controls="chapter-infor-{index}" aria-expanded="true">
						<h3>Chapter {chapter.number}: {chapter.title}</h3>
					</button>
				</li>
			{/each}
		</ul>
		<div class="chapter-infor">
			<h3 class="chapter-strapline italic mb-m">
				{selectedChapterInformation.title}
			</h3>
			<p>
				{selectedChapterInformation.excerpt}
			</p>
		</div>
	</div>
</section>
...
<style>
	...
	.chapter-container .chapter-title {
		...
		padding: 10px;
		text-align: left;
		border-bottom: 3px solid #8888;
	}
	.chapter-container .chapter-title-selected {
		color: white;
		background: black;
		border: none;
		box-shadow:
	...
	.chapter-container .chapter-infor {
		flex: 7;
	}
</style>
